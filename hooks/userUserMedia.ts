...
const useUserMedia = ({ preferEnvironmentCamera = false } = {}) => {
  const preferences: any = usePreferencesStore();
  ...
  const updateUserMedia = useCallback(
    // TODO: verificar tipo
    async (constraints: { audio: any; video: any }) => {
      const _stream = await navigator.mediaDevices.getUserMedia({
